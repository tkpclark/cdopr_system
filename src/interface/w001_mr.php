...
try{
	...
	$log="outTradeNo=".$outTradeNo."&subject=".$subject."&status=".$status."&totalFee=".$totalFee."&appKey=".$appKey."&paymentTime=".$paymentTime;
	
	if(!empty($status) && !empty($outTradeNo)){
		$sql="update wraith_wo_sdk set status='$status',paymentTime='$paymentTime' where outTradeNo='$outTradeNo'";
		if($result = exsql($sql)){
			$output = 1;
		}
	}

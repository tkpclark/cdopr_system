...
try{
	...
	$log="outTradeNo=".$outTradeNo."&subject=".$subject."&description=".$description."&price=".$price."&quantity=".$quantity."&totalFee=".$totalFee."&appKey=".$appKey."&appName=".$appName."&callBackPath=".$callBackPath."&timeStamp=".$timeStamp;

	if(!empty($outTradeNo) && !empty($subject) && !empty($totalFee) && !empty($appKey) && !empty($appName) && !empty($callBackPath) && !empty($timeStamp)){
		$sql="insert into wraith_wo_sdk(outTradeNo,subject,description,price,quantity,totalFee,appKey,appName,callBackPath,timeStamp) values('$outTradeNo','$subject','$description','$price','$quantity','$totalFee','$appKey','$appName','$callBackPath','$timeStamp')";
		if($result = exsql($sql)){
			$output = 1;
		}
	}
